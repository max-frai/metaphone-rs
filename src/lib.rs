...
impl Metaphone {
    pub fn new(lang: Language) -> Self {
        match lang {
            Language::Russian => Metaphone {
                rules: RulesRussian.to_vec(),
            },
            Language::Ukrainian => Metaphone {
                rules: ([&RulesUkrainian.to_vec()[..], &RulesRussian.to_vec()[..]].concat()),
            },
        }
    }
...
mod tests {
    ...
    fn test_ru() {
        ...
        assert_eq!(
            metaphone.get("Шварценеггер"),
            "ШВАРЦИНИГИР"
        );
        assert_eq!(metaphone.get("Взвешенный"), "ФСВИШИНАЙ");
        assert_eq!(
            metaphone.get("Аффилированный"),
            "АФИЛИРАВАНАЙ"
        );
        assert_eq!(
            metaphone.get("Воспользовавшемуся"),
            "ВАСПАЛЗАВАФШИМУСА"
        );
        assert_eq!(
            metaphone.get("Предшествовавшими"),
            "ПРИТШИСТВАВАФШИМИ"
        );
        assert_eq!(
            metaphone.get("Нововодолажского"),
            "НАВАВАДАЛАЖСКАГА"
        );
        assert_eq!(
            metaphone.get("Неиссякаемый"),
            "НИСАКАИМАЙ"
        );
        assert_eq!(
            metaphone.get("Дифференцированная"),
            "ДИФИРИНЦИРАВАНА"
        );
        assert_eq!(
            metaphone.get("Мальденштам"),
            "МАЛДИНШТАМ"
        );
